...
class PlantUMLComponentExporter(Exporter):
    ...
    def write_components_structure(self, root_module: Module, traced_function: Function):
        for component_line in visit_module(root_module, (), 0, traced_function):
...
def visit_module(module: Module, parent_module_path: Tuple[str], indentation_level: int,
                 traced_function: Function) -> Iterable[str]:
    has_functions = len(module.functions) > 0

    # TODO use the formatter to properly handle '__' and '@'
    # TODO handle cases where the root module contains several modules (use rectangle instead of package)
    ...
    if not has_functions and len(module.sub_modules) == 1:
        ...
        yield from visit_module(sub_module, sub_module_parent_path, indentation_level, traced_function)

    # writes the parent package if any, then revisits the current module (without parents, with an increased indentation level)
    elif len(parent_module_path) > 0:
        indentation = indentation_level * INDENT
        yield f"{indentation}package {'.'.join(parent_module_path)} {{"

        yield from visit_module(module, (), indentation_level + 1, traced_function)
    ...
    # writes the module with its functions and sub-modules, or the package with sub-modules
        ...
        # opens the component
        component_type = 'frame' if has_functions else 'package'
        yield f'{indentation}{component_type} {module.name} {{'
        ...
        if len(module.sub_modules) > 0:
            ...
            for sub_module in module.sub_modules.values():
                yield from visit_module(sub_module, (), sub_indentation_level, traced_function)
